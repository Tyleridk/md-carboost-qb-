...
RegisterServerEvent('md-carboost:server:getcopsd', function()
    ...
    if copsOnDuty >= Config.ActivePolice then
        if not timeOut then
            ...
            TriggerClientEvent('QBCore:Notify', src, "You Need 5 Minutes To Order A Boost", "error")
        end    
    else
        TriggerClientEvent('QBCore:Notify', src, 'Need at least '..Config.ActivePolice.. ' police to boost a car', "error")
    end
end)
...
RegisterServerEvent('md-carboost:server:getcopsvin', function()
    ...
    if copsOnDuty >= Config.ActivePolice then
        if not vintimeout then
            ...
            TriggerClientEvent('QBCore:Notify', src, "You Need 5 Minutes To Order A Vin Scratch Or 60 For Successful Vin Scratch", "error")
        end    
    else
        TriggerClientEvent('QBCore:Notify', src, 'Need at least '..Config.ActivePolice.. ' police to Vin Scratch a car', "error")
    end
end)
...
RegisterNetEvent('md-carboost:server:successfuldrop', function(deliveryData, inTime)
    ...
    if not timeOut then
        ...
        TriggerClientEvent('QBCore:Notify', src, "You Need 10 Minutes To Order A Boost", "error")
    end    
end)
...
RegisterNetEvent('md-carboost:server:successfulvin', function()
    ...
    if not vintimeout then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Need at least 60 Minutes After A VinScratch', "error")
    end    
end)
...
RegisterNetEvent('md-carboost:server:SaveCar', function(mods, vehicle, _, plate)
    ...
    if result[1] == nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "You Scratched The Vin", "success")
    else
        TriggerClientEvent('QBCore:Notify', src, "This Car Is Already Owned", "error")
    end
end)